...
resolve_image_extension() {
    ...
    add_yamls="--file docker-compose-haku.yaml"
...
submit_job() {
    ...
    ssh $CLUSTER_LOGIN "cd $CLUSTER_ISAACLAB_DIR && $CMD $CLUSTER_ISAACLAB_DIR/docker/cluster/$job_script_file \"$CLUSTER_ISAACLAB_DIR\" \"michael-isaac-lab-$container_profile\" ${@}"
}
...
case $mode in
    start)
        echo "[INFO] Building the docker image and starting the container michael-isaac-lab-$container_profile in the background..."
    ...
    enter)
        ...
        is_container_running michael-isaac-lab-$container_profile
        echo "[INFO] Entering the existing 'michael-isaac-lab-$container_profile' container in a bash session..."
        pushd ${SCRIPT_DIR} > /dev/null 2>&1
        docker exec --interactive --tty michael-isaac-lab-$container_profile bash
        popd > /dev/null 2>&1
        ;;
    copy)
        # Check that desired container is running, exit if it isn't
        is_container_running michael-isaac-lab-$container_profile
        DOCKER_ISAACLAB_PATH=$(docker exec michael-isaac-lab-$container_profile printenv DOCKER_ISAACLAB_PATH)
        echo "[INFO] Copying artifacts from the 'michael-isaac-lab-$container_profile' container..."
        ...
        # copy the artifacts
        docker cp michael-isaac-lab-$container_profile:${DOCKER_ISAACLAB_PATH}/logs ./artifacts/logs
        docker cp michael-isaac-lab-$container_profile:${DOCKER_ISAACLAB_PATH}/docs/_build ./artifacts/docs/_build
        docker cp michael-isaac-lab-$container_profile:${DOCKER_ISAACLAB_PATH}/data_storage ./artifacts/data_storage
        echo -e "\n[INFO] Finished copying the artifacts from the container."
        popd > /dev/null 2>&1
        ;;
    stop)
        # Check that desired container is running, exit if it isn't
        is_container_running michael-isaac-lab-base-2.2.0-yin
        is_container_running michael-isaac-lab-base-2.2.0-yang
        echo "[INFO] Stopping the launched docker container michael-isaac-lab-$container_profile..."
        pushd ${SCRIPT_DIR} > /dev/null 2>&1
        docker compose --file docker-compose-haku.yaml $add_profiles $add_envs down
        x11_cleanup
        popd > /dev/null 2>&1
        ;;
    push)
        ...
        check_image_exists michael-isaac-lab-$container_profile:latest
        ...
        rm -rf /$SCRIPT_DIR/exports/michael-isaac-lab-$container_profile*
        ...
        APPTAINER_NOHTTPS=1 apptainer build --sandbox --fakeroot michael-isaac-lab-$container_profile.sif docker-daemon://michael-isaac-lab-$container_profile:latest
        # tar image (faster to send single file as opposed to directory with many files)
        tar -cvf /$SCRIPT_DIR/exports/michael-isaac-lab-$container_profile.tar michael-isaac-lab-$container_profile.sif
        # make sure target directory exists
        ssh $CLUSTER_LOGIN "mkdir -p $CLUSTER_SIF_PATH"
        # send image to cluster
        scp $SCRIPT_DIR/exports/michael-isaac-lab-$container_profile.tar $CLUSTER_LOGIN:$CLUSTER_SIF_PATH/michael-isaac-lab-$container_profile.tar
        ;;
    job)
        source $SCRIPT_DIR/.env.base
        # Check if singularity image exists on the remote host
        check_singularity_image_exists michael-isaac-lab-$container_profile
        # make sure target directory exists
